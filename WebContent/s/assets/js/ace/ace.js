...
jQuery(function($) {
	...
	function enableDemoAjax() {		
		...
		var demo_ajax_options = {
			 'close_active': true,
			 'update_active':function(hash,url,method,updateView,self,manual_trigger){
				 ...
				 if(link_element.length > 0){
					 ...
					 if(nav.length > 0){
						 nav.find('.active').each(function(){
							 var $class = 'active';
							 if($(this).hasClass('hover') || /*self.settings.close_active*/$(this).hasClass('close_active')) $class += ' open';
						 ...
						 nav.closest($.parseHTML('.sidebar[data-sidebar-scroll]=true')).each(function(){
							 var $this = $(this);
							 $this.ace_sidebar_scroll('reset');
							 if(manual_trigger == true) $this.ace_sidebar_scroll('scroll_to_active');
						 });
					 };
				 }
				 return link_element;
			 ...
			 'updateView':true,
			 'default_url':'page/view',
			 'content_method':'POST',
			 'extraParams':{
				 
			 },
			'content_data':{},
			'update_breadcrumbs':function(hash,url,link_element,method,updateView){
				var link_text = '';
			...
			'update_title':function(hash,url,link_text,method,updateView,contentArea){
			...
			'content_url':function(hash){
				var path = "/";
				if(hash.match("^page\/view$")){
					path = document.location.pathname;
				}else if(hash.match(/^page\//)){
					path = XY.Util.app.contentPath;
				}else{
					return false;
				}
				//if(path.match(/admin/))
				if(document.location.pathname && document.location.pathname.match(/admin/))
					var endUrl = hash.replace(/^page\//,'');
				if(path.indexOf("/") != -1)
					path += "/";
				return path + endUrl.replace(/^\/page\//,'');
