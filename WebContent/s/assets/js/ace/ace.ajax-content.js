	...
	function AceAjax(contentArea, settings) {
		...
		this.loadUrl = function(hash, cache, manual_trigger) {
			...
			if(typeof url === 'string'){
				if(this.settings.content_method == "POST" && hash.substring(hash.length-5) != "/view"){
					this.post(url, this.settings.content_data, this.settings.updateView,this.settings.extraParams);
				}else{
					this.getUrl(url, hash, manual_trigger);
				}
		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			.done(function(result) {
				...
				if(typeof self.settings.update_active === 'function') {
					link_element = self.settings.update_active.call(null, hash, url, method, updateView,self,manual_trigger);
				}
				...
				if(typeof self.settings.update_title === 'function') {
					self.settings.update_title.call(null, hash, url, link_text, method, updateView, $contentArea);
				}
